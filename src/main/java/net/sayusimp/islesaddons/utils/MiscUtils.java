...
public class MiscUtils {
    ...
    public static void renderAmountText(MatrixStack matrices, ItemStack stack, int x, int y, int z, int amount)
    {
        ...
        if (amount < 100) DrawableHelper.drawCenteredText(matrices, IslesAddonsClient.client.textRenderer, Text.of(String.valueOf(amount)),  x + 8, y + 8, TextColor.parse("white").getRgb());
        else DrawableHelper.drawCenteredText(matrices, IslesAddonsClient.client.textRenderer, Text.of(String.valueOf(amount)),  (int) ((x+8)/scaleRatio), (int) ((y + GUI_OVERLAY_WIDTH_THRESH - (fontHeight/2)) / scaleRatio), TextColor.parse("white").getRgb());
